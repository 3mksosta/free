...
def start(message):
	...
	f = message.from_user.id	if f == SUDO:
		...
		M = types.InlineKeyboardButton('DEV', url='https://t.me/E_D_W')
		...
		me = types.InlineKeyboardButton(text="يوزري",url="https://t.me/E_D_W")

		ch = types.InlineKeyboardButton(text="▶ قناة البوت ◀",url="https://t.me/O_4_5")
...
def masg(call):
	...
	if call.data =="E_D_W":
		...
		while True:
			...
			E_D_W = [h1,h2,h3,h4,h5]

			j = random.choice(E_D_W)
			...
			if response.text.find('If you have <strong>Telegram</strong>, you can contact <a class="tgme_username_link"')>=0 and j not in ban:
				...
				E_D_W = types.InlineKeyboardButton('DEV', url='https://t.me/E_D_W')

				mas.add(A,E,B,R,E_D_W)
